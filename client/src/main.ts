import { dumpBoard } from './engine/board';
...
let actionMode: 'idle' | 'pass' | 'dribble' | 'shoot' = 'idle';


// ----- Node coordinates (PERCENT) -----
const nodeCoordinates = new Map<number, { xPercent: number; yPercent: number }>([
...
function getPixelPosition(xPercent: number, yPercent: number, container: HTMLElement) {
  return {
...
function selectUnit(unitId: string) {
  ...
  if (!unit) return;
  if (unit.ownerId !== game.turnManager.currentPlayer) return;
...
function renderActionPanel() {
  ...
  if (!unit.hasBall || unit.ownerId !== game.turnManager.currentPlayer) return;
...
// ----- Die Roll Animation -----
function showDieRoll(roll: number, onComplete: () => void) {
  ...
  setTimeout(() => {
    ...
    cube.style.transform = rotations[roll as keyof typeof rotations];
...
function handleKickoffChoice(unitId: string) {
...
// ----- Battle Indicator -----
function showBattleIndicator(nodeId: number) {
  const coord = nodeCoordinates.get(nodeId);
  if (!coord) return;
...
function showBattleWinner(winnerId: string, reason: string) {
...
function promptPostBattleMove(winnerId: string) {
  ...
  const originNode = document.querySelector(`[data-node-id="${unit.position}"]`) as HTMLElement;
  if (originNode) {
    originNode.style.outline = '4px solid #f59e0b';
  }
  Array.from(nodesContainer.children).forEach(n => {
    const nodeId = Number((n as HTMLElement).dataset.nodeId);
    ...
    if (isNeighbor && isEmpty) {
      (n as HTMLElement).style.outline = '3px solid rgba(59, 130, 246, 0.8)';
    }
  });
}

// ----- Highlight passable units -----
function highlightPassable(originId: string) {
  Array.from(nodesContainer.children).forEach(n => (n as HTMLElement).style.outline = '');
  Array.from(nodesContainer.children).forEach(n => {
    const nodeId = Number((n as HTMLElement).dataset.nodeId);
    const candidate = Array.from(units.values()).find(u => u.position === nodeId && u.ownerId === units.get(originId)!.ownerId && u.id !== originId);
    if (candidate) (n as HTMLElement).style.outline = '3px solid rgba(34,197,94,0.8)';
  });
}

// ----- Node click -----
nodesContainer.addEventListener('click', ev => {
  const nodeEl = (ev.target as HTMLElement).closest('[data-node-id]') as HTMLElement | null;
  if (!nodeEl || !selectedUnitId) return;
  ...
  if (actionMode === 'pass') {
    ...
    if (!candidate) return console.log('No friendly unit at that node.');
  ...
  if (res?.result === 'battle pending') {
    showBattleIndicator(nodeId);
    return renderPendingBattlePanel();
  }
  if (!res || res.result === 'illegal') return console.log('Move illegal', res);
  renderUnits();
  advanceTurn();
});
...
function renderPendingBattlePanel() {
  ...
  ['dribble', 'pass', 'shoot'].forEach(action => {
    ...
    btn.className = 'px-3 py-1 m-1 rounded bg-slate-700 text-white';
    btn.addEventListener('click', () => {
      finishBattle(game.resolvePendingBattle(action as 'dribble' | 'pass' | 'shoot'));
    });
    battleActions.appendChild(btn);
  });
}

function finishBattle(result: any) {
...
// ----- Pass helper -----
function doPass(originId: string, targetNodeId: number) {
  finishAction(game.handleAction(originId, 'pass', targetNodeId));
}

// ----- Finish action -----
function finishAction(result: any) {
  if (!result) return;
...
function spawnInitialUnits() {
  spawnUnitFromCard('P1', 'S01', 1); spawnUnitFromCard('P1', 'S13', 2); spawnUnitFromCard('P1', 'S15', 3);
  spawnUnitFromCard('P2', 'S02', 12); spawnUnitFromCard('P2', 'S03', 11); spawnUnitFromCard('P2', 'S12', 10);
}
...
(window as any).dribble = () => { if (!selectedUnitId) return; finishAction(game.handleAction(selectedUnitId, 'dribble', -1)); };
(window as any).passTo = (nodeId: number) => { if (!selectedUnitId) return; finishAction(game.handleAction(selectedUnitId, 'pass', nodeId)); };
(window as any).shoot = () => { if (!selectedUnitId) return; finishAction(game.handleAction(selectedUnitId, 'shoot', -1)); };
