...
class Node {
    ...
    constructor(id: number, neighbors: number[], isGK: boolean) {
        this.id = id;
    ...
    addOccupant(unitId: string) {
        this.occupants.add(unitId);
    }

    removeOccupant(unitId: string) {
    ...
    hasEnemy(ownPlayerId: string): boolean {
        for (const occ of this.occupants) {
            if (!occ.startsWith(ownPlayerId)) return true;
        }
        return false;
    }
}
...
function getNeighbors(id: number): Node[] {
    const node = getNode(id);
    if (!node) return [];
    return node.neighbors.map(nid => nodes.get(nid)!).filter(Boolean);
}

function moveUnit(unitId: string, fromId: number, toId: number): boolean {
...
function dumpBoard(): void {
    for (let [id, node] of nodes.entries()) {
        console.log(
            `Node ${id}${node.isGK ? " (GK)" : ""}: [${[...node.occupants].join(", ")}]`
        );
    }
}

function assertNoDuplicateOccupants(): void {
