...
const unit_1 = require("./engine/unit");
const game = new game_1.GameManager();
// DOM references
...
function renderUnits() {
    ...
    for (const unit of unit_1.units.values()) {
        ...
        nodeUnits.forEach((unit, index) => {
            ...
            u.addEventListener('click', ev => {
                ...
                if (actionMode === 'pass' && selectedUnitId) {
                    const origin = unit_1.units.get(selectedUnitId);
                    return;
                }
                selectUnit(unit.id);
            });
...
function renderStamina() {
    ...
    Array.from(unit_1.units.values()).forEach(unit => {
...
function renderActionPanel() {
    ...
    if (!selectedUnitId)
        return;
    const unit = unit_1.units.get(selectedUnitId);
...
function showCoinToss() {
    ...
    setTimeout(() => {
        ...
        const unitsToChoose = Array.from(unit_1.units.values()).filter(u => u.ownerId === winner);
...
function showBattleWinner(winnerId, reason) {
    const winnerUnit = unit_1.units.get(winnerId);
    if (!winnerUnit)
        return;
    const container = document.createElement('div');
...
function promptPostBattleMove(winnerId) {
    const unit = unit_1.units.get(winnerId);
    const container = document.createElement('div');
...
function highlightPassable(originId) {
    ...
    Array.from(nodesContainer.children).forEach(n => {
        const nodeId = Number(n.dataset.nodeId);
        const candidate = Array.from(unit_1.units.values()).find(u => u.position === nodeId && u.ownerId === unit_1.units.get(originId).ownerId && u.id !== originId);
        if (candidate)
...
nodesContainer.addEventListener('click', ev => {
    ...
    const origin = unit_1.units.get(selectedUnitId);
    if (actionMode === 'pass') {
        const candidate = Array.from(unit_1.units.values())
...
function renderPendingBattlePanel() {
    ...
    const attacker = unit_1.units.get(attackerId);
    const defender = unit_1.units.get(defenderId);
...
function finishAction(result) {
    ...
    if (result.result === 'goal') {
        ...
        const firstUnit = Array.from(unit_1.units.values()).find(u => u.ownerId === kickoffTeam);
        if (firstUnit) {
...
function spawnInitialUnits() {
    (0, unit_1.spawnUnitFromCard)('P1', 'S01', 1);
    (0, unit_1.spawnUnitFromCard)('P1', 'S13', 2);
    (0, unit_1.spawnUnitFromCard)('P1', 'S15', 3);
    (0, unit_1.spawnUnitFromCard)('P2', 'S02', 12);
    (0, unit_1.spawnUnitFromCard)('P2', 'S03', 11);
    (0, unit_1.spawnUnitFromCard)('P2', 'S12', 10);
}
// ----- Resize -----
...
startGame();
console.log('Current units Map:', Array.from(unit_1.units.entries()));
